...
async function resolveApplication(finding: any, allowDeep = false): Promise<RemediationResolution> {
  ...
  if (query) {
    ...
    // Deep pass — broader query expansion + package page inspection (slow, opt-in only)
    if (allowDeep) {
      try {
