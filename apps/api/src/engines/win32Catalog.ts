...
export function buildBundleFiles(record: Win32ResolvedRecord) {
  ...
  const readme = [
    ...
    '',
    '## What to enter in Intune',
    `- App type: Windows app (Win32)` ,
    `- App name: ${record.name}` ,
    `- Publisher: ${record.publisher}` ,
    `- Install command: powershell.exe -ExecutionPolicy Bypass -File install.ps1` ,
    `- Uninstall command: powershell.exe -ExecutionPolicy Bypass -File uninstall.ps1` ,
    '- Install behavior: System (change only if the app is intentionally per-user).',
    '- Device restart behavior: Determine behavior based on return codes.',
    '- Detection rules: Use Custom detection script and upload detect.ps1.',
    '- Requirement rules: Start with x64 + minimum OS version that matches your environment.',
    '',
  ...
  const checklist = [
    ...
    '- Install command: powershell.exe -ExecutionPolicy Bypass -File install.ps1',
    '- Uninstall command: powershell.exe -ExecutionPolicy Bypass -File uninstall.ps1',
    '- Install behavior: System unless the package is intentionally per-user.',
    '- Device restart behavior: Determine behavior based on return codes.',
    '',
    '## Requirements',
    '- Start with 64-bit architecture unless the installer is x86 only.',
    '- Set a minimum OS version that matches your tenant standard.',
    '',
    '## Detection',
    '- Use Custom detection script.',
    '- Upload detect.ps1 exactly as generated, then validate on clean and installed devices.',
    '',
    '## Return codes',
    '- 0 = Success',
    '- 3010 = Soft reboot required',
    '- 1641 = Hard reboot initiated',
    '- Add vendor-specific codes if the installer uses them.',
    '',
    '## Validation',
    ...
    '- Capture screenshots or notes for helpdesk handoff if needed.'
  ].join('\n');
